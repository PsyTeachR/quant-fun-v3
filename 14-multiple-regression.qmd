...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `14 Multiple Regression`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_14_multiple_regression` folder. 
...
The same as previous chapters like ANOVA and factorial ANOVA, we can use inline code to help with the write-up. First, copy and paste the code below into **white-space** in your Quarto document and then render. Note that we enter the *p*-values manually because of the APA "*p* < .001" formatting.
