...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `09 t-tests and Regression`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_09_regression_categorical` folder. 
...
lopez_clean <- lopez_data %>% 
  mutate(Condition = as.factor(Condition),
         Condition_label = recode_values(Condition,
                                         "0" ~ "Control",
                                         "1" ~ "Experimental"))
```
...
### Reporting your t-test reproducibly
...
```{r}
#| code-fold: true
#| code-summary: "Show the t-test formatting function"
...
You can add that function as inline code to insert into your report to make your inferential statistics reproducible. Just note it's difficult to apply all the APA format details, so you might need to remove leading zeros, add italics to the math symbols etc. 
...
```{r}
#| code-fold: true
#| code-summary: "Show the Mann-Whitney formatting function"
...
Here, we repeat our warning about statistical tests to check assumptions. The two most common are the Shapiro-Wilk for normality of residuals and Levene's test for homoscedasticity. If they worked perfectly, they would be great but they have more limitations than benefits. 
...
If you are interested, the posterior predictive check here provides an insight into why we get potential problems with normality. 
...
This process has the benefit of directly producing your effect size, as the intercept estimate is the deviation from your fixed value (here, 50). As we calculated manually before, the erotic hit rate is 3.14% (95% CI = [0.66, 5.62]) above your fixed value. If you remember back to the linear model explanations, this is where the *p*-value for the intercept is finally useful as it tests against 0. 

If you compare to the one-sample t-test, all of these values are identical. You also have the option of calculating a standardised effect size, and checking assumptions by applying the previous linear regression activities.  
...
The output is almost identical to the one-sample t-test, but this time the effect size is the mean difference between conditions, not just the mean per condition. Behind the scenes, a paired-samples t-test is actually a one-sample t-test in a trenchcoat as it uses the difference between conditions as the outcome. The difference between the two conditions was not statistically significant. 
...
This process directly produces your effect size again, as the intercept estimate is the deviation from 0 for your difference score. As we saw in the paired-samples t-test output, there was a 3.31% (95% CI = [-0.23, 6.84]) higher hit rate for erotic images compared to control (as we calculated erotic - control), but the difference was not statistically significant. 

If you compare to the paired-samples t-test, all of these values are identical. You also have the option of calculating a standardised effect size, and checking assumptions by applying the previous linear regression activities.    
...
At this point, we encourage you to complete the second data analysis journey chapter we designed for this milestone - [Data Analysis Journeys: Evans et al. (2023)](https://psyteachr.github.io/analysis_journeys/Evans-analysis.html){target="_blank"}. This is a bridge between the structured learning in these chapters and your assessments. Like the data wrangling journey chapter, you have more of an unstructured task to complete and test your understanding against the activities. Of course, if you need any help, there are hints and the solution available. 
