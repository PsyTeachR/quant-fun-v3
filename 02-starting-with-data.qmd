...
In this chapter, we introduce you to using code to create **`r glossary("reproducible research")`**. Creating reproducible research means you will write text and code that completely and transparently performs an analysis from start to finish in a way that produces the same result for different people using the same software on different computers. We will cover things such as file structure and setting a working directory, using **Quarto** files, and writing code chunks. 
...
In chapter 1, we never worked with files, so you did not have to worry about where you put things on your computer. Before we can start working with Quarto files, we must explain what a **`r glossary("working directory", def = "The filepath where R is currently loading files from and saving files to.")`** is and how your computer knows where to find things. 
...
#### File management when using the online version of R/RStudio

The main disadvantage to using the R/RStudio online is that you will need create folders online and then upload and download any files you are working on. Please be aware that **there is no link between your computer and the online version of R**. If you change files online, they will not appear on your computer until you download them, and you need to be very careful when you submit your assessment files that you are submitting the right file. This is the main reason we recommend installing R / RStudio on your computer wherever possible.

Going forward throughout this book, you will need to follow an extra step where you also upload them online. As an example: 

1. In the file pane, click `New folder` and create the same structure we demonstrated above.

2. Download <a href="data/ahi-cesd.csv" download>`ahi-cesd.csv`</a> and <a href="data/participant-info.csv">`participant-info.csv`</a> into the `data` folder you created for Chapter 2. To download a file from this book, right click the link and select "save link as". Make sure that both files are saved as ".csv". Do not open them on your machine as often other software like Excel can change setting and ruin the files.

3. Now that the files are stored on your computer, go to R/RStudio online and click `Upload` then `Browse` and choose the folder for the chapter you are working on.

4. Click `Choose file` and go and find the data you want to upload.
:::
...
## Creating and navigating Quarto documents

Throughout this data skills book and related assignments, you will use a file format called Quarto (abbreviated as .Qmd) which is a great way to create dynamic documents combining regular text and embedded code chunks. Learning how to use these documents provides a versatile toolkit. We only cover creating documents in this book but James has prepared a bunch of materials on using Quarto to make books (like the one you are working with now), websites, and presentations: [Dissemination using Quarto and Github Pages](https://bartlettje.github.io/dissemination_quarto/){target="_blank"}. 

Quarto documents are self-contained and fully reproducible, meaning if you have the necessary data, you should be able to run someone else's analyses. This is an important part of your open science training as one of the reasons we teach data skills this way is that it enables us to share open and reproducible information. 
...
For more information about Quarto, there is extensive documentation on [https://quarto.org/docs/guide/](https://quarto.org/docs/guide/){target="_blank"}, but for now the key advantage to know about is that it allows you to combine code and text in a document and then **render** the file to create a webpage (**`r glossary("HTML", display = "html")`**), a PDF, or Word document (.docx).

### Activity 3: Open, save, and render a new Quarto document {#intro-a3}

Open a new Quarto document by clicking the 'new item' icon and then click 'Quarto' like @fig-img-new-markdown.

::: {#fig-img-new-markdown layout-ncol=2}

![](images/create_qmd_menu.png)

![](images/create_qmd_options.png)

Creating a new Quarto document from the menu (left) and setting title, author, date, and output (right). 
:::

After selecting Quarto, there are different format options you can explore in time, but Select Document and there are three boxes to complete: 
...
- Author: This is where you can add your name or names for multiple people. For this chapter, enter your GUID as this will be good practice for the data skills assessments.
...
Once you click **Create**, this will open a new Quarto document.

Save this Quarto document by clicking **`File >> Save as`** from the top menu, and name this file "`02_reproducible_docs`". Note the document title and file name are separate, so you still have to name the file when you save it. 
...
#| fig.cap: "New .Qmd file in your working directory."

knitr::include_graphics("images/files_new_qmd.png")

```

Now you have the default version of the Quarto file, you have a bunch of text and code to show its capabilities (@fig-img-markdown-default).

```{r img-markdown-default, echo=FALSE, fig.cap="New Quarto text"}
#| label: fig-img-markdown-default
#| fig.cap: "Default text and code in a new Quarto document."

knitr::include_graphics("images/default_qmd.png")

```

We will now demonstrate what it looks like to **Render** a document. This means that we are going to compile (i.e., turn) our code into a document that is more presentable. This way, you can check it renders and there are no errors. So, as we add changes in the following activities, you can identify if and when any errors appear and fix them quicker. 

At the top of your Quarto window, you should see a Render button (@fig-img-render-document). If you click that, the document will render and produce a html file. 

```{r img-render-document, echo=FALSE}
#| label: fig-img-render-document
#| fig.cap: "Clicking the render button on a .Qmd document."

knitr::include_graphics("images/render_document.png")

```

You will see a small version of the rendered document appear in the Viewer tab in the bottom right of your screen. You will also see it has created a new file in your working directory. It will have the same name as your Quarto file, but with .html as the file ending. You can view the rendered document by clicking the "Show in new window" button or opening the file from your folder (@fig-img-rendered-default). This should open the document in your default internet browser as websites are created in html. 

::: {#fig-img-rendered-default layout-ncol=2}

![](images/rendered_default_files.png)

![](images/rendered_default_browser.png)

On the left, you can see a small version of the rendered document in the Viewer tab. On the right, you can see the full version open in your internet browser.
:::

Now everything is working and rendering, we can start editing the Quarto file to add new content. 

::: {.callout-note}
#### Changing the default viewer

These are all the default options when you use RStudio. It can be quite annoying to see the preview in the tiny viewer pane, so James prefers to make the default opening in the browser window: `Tools >> Global Options... >> RMarkdown >> Show output preview in... >> Window.` 
:::

### Activity 4: Create a new code chunk {#intro-a4}

Let's start using your new Quarto document to combine code and text. Follow these steps: 

1. Delete **everything** below line 6. 

2. On line 7, type "## About me". 

3. With your cursor on line 9, insert a blank code chunk by clicking on the top menu **`Code >> Insert Chunk`** or using the shortcut at the top of the Quarto window that looks like a small green plus and the letter c and select R. 
...
#| fig.cap: "Creating a new R chunk in your blank Quarto document"

knitr::include_graphics("images/qmd_blank_chunk.png")

```

What you have created is called a **`r glossary("chunk", display = "code chunk")`**. Quarto assumes anything written outside of a code chunk is just normal text, just like you would write in a text editor like Word. It assumes anything written inside the code chunk is R code. This makes it easy to combine both text and code in one document.
...
One common mistake is to accidentally delete one or more of these back ticks. A useful thing to notice is that code chunks tend to have a different color background - in the default appearance of RStudio a code chunk is grey and the normal text is white. You can use this to look for mistakes. If the code chunks keep expanding it's grey area as you return new lines, check that you have not deleted the backticks. RStudio essentially thinks the code chunk has not finished yet. 
...
::: {.callout-note}
#### Markdown language
When you typed "## About me", you might notice the two hashes. You will see the effect of this shortly, but this is using **`r glossary("Markdown")`** language to add document formatting. Markdown is a type of formatting language, so instead of using buttons to add features like you would in Word, you add symbols which will produce different features when you render the document.

The hashes create headers. One (#) creates a first level header (larger text), two (##) creates a second level header, and so on. Make sure there is a space between the text and hash or it will not render properly. 

As we progress through the book, we will slowly introduce you to different Markdown features, but you can see the [Quarto Markdown Basics page](https://quarto.org/docs/authoring/markdown-basics.html){target="_blank"} if you are interested.
:::

### Activity 5: Write some code {#intro-a5}

Now we are going to use the code examples you read about in Chapter 1 - [Introduction to programming with R/R Studio](#C01-programming-basics) - to add some code to our Quarto document. 
...
age <- 32
today <- Sys.Date()
next_birthday <- as.Date("2027-02-18") # Year, month, day format
```
...
When you are working in an Quarto document, there are several ways to run your lines of code.
...
Your code works and you now know how to run it, but one of the incredible benefits we said about Quarto is that you can mix text and code. Even better is the ability to combine code with text to put specific outputs of your code, like a value, using **`r glossary("inline code", def = "Directly inserting the result of code into the text of a .Qmd file.")`**. 
...
Your .Qmd should look like @fig-img-aboutme-rmd but nothing will happen yet. Unlike code chunks, you cannot run inline code. You need to render your document for it to do it's magic. 

```{r img-aboutme-rmd, echo=FALSE}
#| label: fig-img-aboutme-rmd
#| fig.cap: "Complete .Qmd file with your about me section and inline code."

knitr::include_graphics("images/qmd_aboutme.png")
...
You will see it has the form of a backtick (`` ` ``), `r` and a space, the object/function you want to reference, and a final backtick. When the Quarto file renders, it sees the `r` and recognises it as inline code and uses the object or function. 

If you just write the two backticks without the `r`, it will just add code formatting and not produce inline code.
:::

### Activity 8: Rendering your file {#intro-a8}

As our final step in this part, we are going to render our file again to see how it looks now. So, click the Render button to regenerate your rendered .html version.

If you look at the rendered .html document in the Viewer tab or in your browser, you should see the sentence we copied in from Activity 7. As if by magic, that slightly odd bit of text you copied and pasted now appears as a normal sentence with the values pulled in from the objects you created:

My name is **`r name`** and I am **`r age`** years old. <br><br>It is **`r next_birthday - today`** days until my birthday.

Quarto is an incredibly powerful and flexible format, we wrote this whole book using it! There are a few final things to note about rendering that will be useful going forward for your data skills learning and assessments:

- Quarto will only render if your code works. If you have an error, it will stop and tell you to fix the error before you can click Render and try again. This is a good way of checking whether you have written functioning code in your assessments. There is actually a way of ignoring errors we use in data skills assessments, but we want you to recognise errors at this point. 

- When you render an Quarto document, it runs the code from the start of the document to the end in order, and in a fresh session. This means it cannot access your environment, just the objects you create within that Quarto. One common error can be writing and running code as you work on the document, but the code chunks are in the wrong order, or you created an object in the console but not in the code chunks. This means Quarto would not know the object exists yet, or it does not have access to it at all. 

- You can choose to render to a Word document rather than HTML. This can be useful for sharing with others or adding further edits, but you might lose some functionality. By default, html looks good and is accessible, so that will be our default throughout this book, but look out for our instructions on what output format we want your assessments in. 

- You can choose to render to PDF, however, this requires a **`r glossary("LaTeX")`** installation and can be quite complicated. If you do not already know what LaTeX is and how to use it, we do not recommend trying to render to PDF just yet. If you do know how to use LaTeX, you probably do not need us to give you instructions!

We will test some of these warnings in error mode in the test yourself section, but we have one final demonstration for how Quarto files support reproducibility. 

## Demonstrating reproducibility 

At the start of this chapter, we plugged the benefits of reproducible research as the ability to produce the same result for different people using the same software on different computers. We are going to end the chapter on a demonstration of this by giving you a Quarto document and data. You should be able to click Render and see the results without editing anything. We do not expect you to understand the code included in it, we are previewing the skills you will develop over the next few chapters on visualisation and data wrangling. 

### Activity 9 - Render the reproducibility demonstration document

Please follow these steps and you should be able to render the document without editing anything. Make sure you are still in your `Chapter_02_reproducible_docs` folder. If you are coming back to this activity, remember to set your working directory by opening the .Rproj file.  
...
At this point, you should have "**02_reproducibility_demo.Qmd**" within your `Chapter_02_reproducible_docs` folder. You should have "**ahi-cesd.csv**" and "**participant_info.csv**" in the `data/` folder within `Chapter_02_reproducible_docs`. 

If you open "02_reproducibility_demo.Qmd" and followed all the steps above, you should be able to click Render. This will turn the Quarto file into a rendered html file, showing some data wrangling, summary statistics, and two graphs (@fig-img-repro-demo). In the next chapter, you will learn how to write this code yourself, starting with creating graphs.

::: {#fig-img-repro-demo layout-ncol=2}

![](images/repro_demo_qmd.png)

![](images/repro_demo_html.png)

You should see the reproducibility demonstration as the .Qmd (left) and be able to render it into a html file (right).
:::
...
**Question 2**. When using the default environment color settings for RStudio, what color would the background of a code chunk be in Quarto? `r mcq(c("white","red","green",answer = "grey"))`

**Question 3**. When using the default environment color settings for RStudio, what color would the background of normal text be in Quarto? `r mcq(c(answer = "white","red","green","grey"))`
...
The following questions are designed to introduce you to making and fixing errors. For this topic, we focus on Quarto and potential errors in using code blocks and inline code. Remember to keep a note of what kind of error messages you receive and how you fixed them, so you have a bank of solutions when you tackle errors independently. 

Create and save a new Quarto file by following the instructions in [activity 3](#intro-a3) and [activity 4](#intro-a4). You should have a blank Quarto file below line 6. Below, we have several variations of a code chunk and inline code errors. Copy and paste them into your Quarto file, click Render, and look at the error message you receive. See if you can fix the error and get it working before checking the answer.

**Note**: For all of our error mode activities, the error is based on clicking Render so it applies to inline code examples. If you try and run the code instead of clicking Render on the whole document, you might see a different error message to what we report. 

**Question 6**. Copy the following text/code/code chunk into your Quarto file and press render. You should receive a few lines of error message containing `Error in eval(parse_only(code), envir = envir) : object 'city' not found` in the Background jobs tab. 
...
Here, we wrote `city <- "Glasgow"` outside the code chunk. So, when we try and render, it is not evaluated as code, and city does not exist as an object to be referenced in inline code. If you copy `city <- "Glasgow"` into the code chunk and press render, it should work. 
:::

**Question 7**. Copy the following text/code/code chunk into your Quarto file and press render. This...works? 
...
Here, we missed a final backtick in the code chunk. You might have noticed all the text had a grey background, so Quarto thought everything was code. Quarto can be quite robust to errors, so it "worked", but it did not render the inline code. When it reached the next back tick around `r` `city`,  it used that to finish off the code chunk. If you add the final backtick to the code chunk, you should be able to click render successfully and see the inline code work this time. 
:::

**Question 8**. Copy the following text/code/code chunk into your Quarto file and press render. You should receive an error like `Error in eval(parse_only(code), envir = envir) : object 'city' not found` in the Background jobs tab. 
...
Here, we tried using inline code before the code chunk. Quarto runs the code from start to finish in a fresh environment. We tried referencing `city` in inline code, but Quarto did not know it existed yet. To fix it, you need to move the inline code below the code chunk, so you create `city` before referencing it in inline code. 
:::

**Question 9**. Copy the following text/code/code chunk into your Quarto file and press render. This...works? 
...
Here, we have a sneaky kind of "error" where it renders, but it is not doing what we wanted it to do. In the inline code part, we only added code formatting `city`, we did not add the r to get Quarto to interpret it as R code: 

```{r eval=FALSE}
`r city`
```

If you add the `r` after the first backtick, it should render and add the city object in. 
:::
