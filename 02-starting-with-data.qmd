...
At the top of your Quarto window, you should see a Render button next to a little ball of yarn (@fig-img-render-document). If you click that, the document will render and produce a html file. 
...
3. With your cursor on line 14, insert a blank code chunk by clicking on the top menu **`Code >> Insert Chunk`** or using the shortcut at the top of the Quarto window that looks like a small green c and select R. 
...
- Quarto will only render if your code works. If you have an error, it will stop and tell you to fix the error before you can click Render and try again. This is a good way of checking whether you have written functioning code in your assessments. 
...
- You can choose to render to PDF, however, unless you are using the server this requires a **`r glossary("LaTeX")`** installation and can be quite complicated. If you do not already know what LaTeX is and how to use it, we do not recommend trying to render to PDF just yet. If you do know how to use LaTeX, you probably do not need us to give you instructions!
...
**Question 6**. Copy the following text/code/code chunk into your Quarto file and press render. You should receive an error like `Line 18 Execution halted` in the Render tab. 
...
**Question 7**. Copy the following text/code/code chunk into your Quarto file and press render. You should receive an error like `Error in parse(): ! attempt to use zero-length variable name` which is not very helpful for diagnosing the problem. 
...
Here, we missed a final backtick in the code chunk. You might have noticed all the text had a grey background, so Quarto thought everything was code. So, when it reached the inline code and text, it tried interpreting it as code and caused the error. If you add the final backtick to the code chunk, you should be able to click render successfully. 
:::

**Question 8**. Copy the following text/code/code chunk into your Quarto file and press render. You should receive an error like `Line 12 Execution halted` in the Render tab. 
