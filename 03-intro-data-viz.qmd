...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `03 Introduction to Data Visualisation`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_03_intro_data_viz` folder. 
...
If you use RStudio online, remember that working with files is a little different.

The main disadvantage to using R online is that you will need create folders and then upload and download any files you are working on. Please be aware that **there is no link between your computer and the online version of R/RStudio**. If you change files online, they will not appear on your computer until you download them, and you need to be very careful when you submit your assessment files that you are submitting the right file. This is the main reason we recommend installing R / RStudio on your computer wherever possible.

Going forward throughout this book, if you are using RStudio online, you will need to follow an extra step where you also upload them online. As an example: 

1. Log into RStudio online.
...
4. Now that the files are stored on your computer, go to RStudio online and click `Upload` then `Browse` and choose the folder for the chapter you are working on.
...
To load the `tidyverse`, below line 6 of your Quarto document, create a code chunk, type the following code into your code chunk, and run the code:
...
::: {.callout-tip}
#### Quarto tip of the chapter: Add code comments
...
In the code chunk above, we added a code `r glossary("comment")` by adding a hash (`#`). In code chunks and scripts, you can add a comment which R will ignore, so you can explain to yourself what the code is doing. In Quarto, you can combine adding notes to yourself outside and inside the code chunks. 
...
Copy and run the following code in your Quarto document, at least below where you read and wrangled the data: 
...
Now you are familiar with the layering system, we will jump straight into creating the barplot. As before, type and run the code in each step, making notes to yourself either in the Quarto document outside the code chunks, or using code comments. 
...
Create and save a new Quarto file for these activities by following the instructions in Chapter 2. You should have a blank Quarto file below line 6. Below, we have several variations of a code chunk and inline code errors. Copy and paste them into your Quarto file, click Render, and look at the error message you receive. See if you can fix the error and get it working before checking the answer.

**Question 4**. Copy the following code chunk into your Quarto file and press Render. You should receive an error like `Error in read_csv(): ! could not find function "read_csv"`. 
...
If you only added this code chunk in, you have not loaded tidyverse yet. Remember Quarto renders from start to finish in a fresh session, so it will not work even if you have loaded already tidyverse outside the Quarto document. So, you would need to add `library(tidyverse)` first. 
:::

**Question 5**. Copy the following code chunk into your Quarto file and press Render. You should receive an error like `! participant-info.csv does not exist in current working directory`. 
...
**Question 6**. Copy the following code chunk into your Quarto file and press Render. You should receive a long error where the problem is buried in the first five lines: 
...
**Question 7**. Copy the following code chunk into your Quarto file and press Render. This...works, but does not look quite right? 
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. You should receive a long error again with lines 5-7 key: 
