...
In contrast to previous chapters, there will be no data wrangling in this chapter, so we do not need to worry about downloading files. We will still be working around some key studies, but we will introduce them as needed. However, you will still be working in an Quarto document, so you need to make sure your working directory is in order.
...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `10 Statistical Power and Effect Sizes`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_10_power` folder. 

4. In a code chunk, load the <pkg>pwr</pkg> and <pkg>tidyverse</pkg> packages. If you need to install any packages, revisit [Chapter 1](#install-tidy) if you need a refresher, but remember not to install packages on the university computers. 
...
The branch of statistics we are using here is Null Hypothesis Significance Testing (NHST). There are two types of `r glossary("hypothesis", display = "hypotheses")` and what you are trying to establish is the `r glossary("probability")` of your data - assuming that the null hypothesis is true - to make a decision. Those two hypotheses are:
...
2. **Sensitivity** power analysis: What effect size can I detect, for a given alpha, beta/power, and sample size? This is most useful after you finish collecting data or when you are using secondary data as the sample size is no longer under your control and it helps put your findings in context. 

::: {.callout-warning}
#### Beware of post-hoc power 

You may now be thinking though, if there is a relationship between all four concepts, could we calculate power for a given alpha, sample size, and effect size? It is a tempting idea and you might see some articles report it, but unfortunately it tells you nothing more than the *p*-value. 

The short version is we are typically using a sample to learn something about a population, so there is uncertainty in the estimate. Using the observed effect size in your study assumes this is the true population effect, which is rarely a good assumption. If you are interested, @lakens_sample_2022 is a great resource for sample size justification in general, but also explains why post-hoc power is not a good idea. Friends do not let friends report post-hoc power. 
:::
...
::: {.callout-important}
#### What if I want to calculate power for non-parametric tests? 

Unfortunately, the <pkg>pwr</pkg> package does not include out-of-the-box functions for calculating power for tests like the Spearman. Really, you need to simulate a power analysis for these test but it is beyond the scope of this course. James explored this though for a Spearman correlation and you can see in @fig-img-pearson-spearman-grid a grid comparing sample sizes for the same settings between a Pearson and Spearman correlation. You consistently have greater power (or need fewer participants) for a Pearson correlation but the difference is neglible once the smallest effect size of interest approaches *r* = 0.5. 

```{r pearson-spearman-grid, echo=FALSE}
#| label: fig-img-pearson-spearman-grid
#| fig.cap: "Comparing the statistical power between a Pearson and Spearman correlation. Each panels shows a different effect size from r = 0.1 to 0.9."

knitr::include_graphics("images/Pearson Spearman grid.png")
```
...
Create and save a new Quarto file for these activities. Delete the example code, so your file is blank from line 6. Create a new code chunk to load the packages <pkg>tidyverse</pkg> and <pkg>pwr</pkg>.

Below, we have several variations of a code chunk error or misspecification. Copy and paste them into your Quarto file below the code chunk to load <pkg>tidyverse</pkg> and <pkg>pwr</pkg>. Once you have copied the activities, click Render and look at the error message you receive. See if you can fix the error and get it working before checking the answer.

**Question 7**. Copy the following code chunk into your Quarto file and press Render. You should get a cryptic sounding error like `Error in uniroot(function(n) eval(p.body) - power, c(4 + 1e-10, 1e+09)): f() values at end points not of opposite sign`. 
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. You want to conduct a sensitivity power analysis when you have unequal sample sizes. You should get the following error: `Error in pwr.t.test(n1 = 40, n2 = 50, d = NULL, sig.level = 0.05, power = 0.9,  : unused arguments (n1 = 40, n2 = 50)`. 
