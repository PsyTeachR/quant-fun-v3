...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `12 ANOVA`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_12_ANOVA` folder. 
...
Now we can run the one-way ANOVA using `aov_ez()` from the <pkg>afex</pkg> package and save it to the object `mod`.
...
- Is the overall effect of Condition significant? `r mcq(sample(c(answer = "Yes", "No")))`

- What is the F-statistics to 2 decimal places? `r fitb("3.79")`

- According to the rules of thumb, the effect size is `r mcq(sample(c("Small", "Medium", answer = "Large")))`
...
If you add that code to your Quarto document, rendering it should create the following: 
