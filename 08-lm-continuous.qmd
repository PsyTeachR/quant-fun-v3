...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `08 Correlations and Regression`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_08_regression_continuous` folder. 
...
1. Load the following packages (three of these are new, so revisit [Chapter 1](#install-tidy) if you need a refresher of installing R packages, but remember not to install packages on the university computers.):
...
| fairness_satisfaction | `r typeof(dawtry_clean$fairness_satisfaction)`| Perceived fairness and satisfaction about the current system of wealth redistribution: Mean of two items (1 extremely unfair – 9 extremely fair). In the original paper, the authors explain these items were reverse-coded but it looks like only the labels were switched. So, higher values mean greater perceived fairness and satisfaction with the current system. |
...
::: {.callout-caution collapse="true"}
#### Show me the solution
The scatterplot shows a negative correlation between the two variables. You need to be careful interpreting fairness and satisfaction due to the sneaky reverse-coding. Higher values mean *greater* fairness and satisfaction. 

As support for wealth redistribution increases up the y-axis to be more positive, perceived fairness and satisfaction tends to decrease on the x-axis. In other words, for people who perceive things to be fair and they are more satisfied, they are less supportive of reforms to wealth redistribution. 
...
By default, these functions will apply a two-tailed test to look for any correlation value more extreme than zero - the null hypothesis - which could be positive or negative. If you wanted to run a one-tailed correlation and look for an effect in one specific direction, you would use the argument `alternative = "greater"` for positive correlations and `alternative = "less"` for negative correlations. 
:::
...
For our inferential statistics, the relationship is not statistically significant and the Pearson correlation coefficient is very weak, *r* (302) = -0.03, *p* = .625, 95% CI = [-0.14, 0.08]. Note there is a missing value for age, so we have one fewer participant / degrees of freedom. 
...
### Reporting your correlation reproducibly
...
If you look in the Environment, the <pkg>correlation</pkg> package helpfully saves the output as a data frame which makes it easy to work with. For the following code, this is a function we have created, so we do not expect you to understand everything at this point. You just need to copy and paste it into your .qmd file before you want to reference the correlation. Click show the correlation formatting function as we initially hide it to avoid taking up lots of space. 

```{r}
#| code-fold: true
#| code-summary: "Show the correlation formatting function"
...
You can add the function as inline code to insert into your report to make your inferential statistics reproducible. Just note it's difficult to apply all the APA formatting details, so you might need to remove leading zeros, add italics to the math symbols etc. 
...
### Activity 5 - Using the `lm()` function 
...
You can see the connection between the two by comparing the value of Pearson's *r* from Activity 4 (-.70) to the value of $R^2$ = .4905. If you take the square root to get *r* (`sqrt(.4905)`), you get .70, which is exactly the same absolute value since $R^2$ can only be positive. So, when you have a single continuous predictor, it is the exact same process as correlation, just expressed slightly different. 

In psychology, most textbooks and journal articles will use these specialised versions of linear regression as a toolbox of statistical tests for different designs. We prefer grounding things in a linear models approach as it provides a more flexible toolkit you can adapt to your design. However, we still want to explain the other tests, so that you can understand them when you come across them in your research. 
:::

### Activity 6 - Calculating confidence intervals
...
### Activity 7 - Centering and standardising predictors
...
Reassuringly, we get the same values as the manual process above.

:::

## Checking assumptions

For the inferential statistics to work as intended, the model makes certain assumptions about the data you are putting into it and the accuracy of the inferences depends on how sensible these assumptions are. Remember these functions will always work even if the numbers you enter are nonsense, so it's important for you as the researcher to recognise when it's appropriate to use these techniques and when it is not.

### Activity 8 - Diagnostic plots for linear regression
...
::: {.callout-warning}
#### Avoid assumption tests like the plague

In your reading, you might see individual statistical tests to check assumptions. The two most common are the Shapiro-Wilk for normality of residuals and Levene's test for homoscedasticity. If they worked perfectly, they would be great but they have more limitations than benefits. 

They flip standard NHST logic to treat the null hypothesis as say the residuals are perfectly normal. The larger the difference between the null and your observed residuals, the more likely the test is to be statistically significant. This means if the test is statistically significant, the logic goes you reject the null and conclude the residuals **are not** normal. However, because it is based on NHST, there are two major fallacies. First, *p*-values are sensitive to sample size, so when you have a large sample size, almost any minor variation would be statistically significant, even when there is not a problem. Second, for a non-significant result, you cannot conclude the null is true, just that you cannot reject the null. As attractive an idea as they are, we heavily recommend avoiding the assumption tests. 

:::

We want to walk through this longer way of checking assumptions to develop a solid understanding before showing you a shortcut to see them all below. In the code, we use a format you will be less familiar with as all these functions come from base R. If you just run the code for the diagnostic plots `plot(lm_redistribution)`, each one gets individually printed to your Plots window. Here, we create a 2x2 panel to show them all together. 
...
### Formatting your results reproducibly
...
```{r}
#| code-fold: true
#| code-summary: "Show the regression model formatting function"
#| 
...
```{r}
#| code-fold: true
#| code-summary: "Show the regression slope formatting function"
#| 
...
Create and save a new Quarto file for these activities. Delete the example code, so your file is blank from line 6. Create a new code chunk to load <pkg>tidyverse</pkg> and wrangle the data files: 
...
Below, we have several variations of a misspecification. Copy and paste them into your Quarto file below the code chunk to wrangle the data. Once you have copied the activities, click Render and look at the output you receive. See if you can identify the mistake and fix it before checking the answer.

**Question 6**. Copy the following code chunk into your Quarto file and press Render. We want to create a simple linear regression model to specify `fairness_satisfaction` as our outcome variable and `Political_Preference` as our predictor. Have we expressed that accurately? 
...
**Question 7**. Copy the following code chunk into your Quarto file and press Render. We want to standardise the outcome and predictors, so that we get the intercept and slope estimates in standard deviations. Have we expressed that accurately? 
