...
3. Create a new Quarto document and give it a sensible title describing the chapter, such as `07 Scatterplots Boxplots Violins`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_07_dataviz` folder. 
...
4. There is currently no identifier, so create a new variable called `participant_ID`. Hint: explore whether you can find a function to add a row number.  
...
zhang_data <- zhang_data %>%
  ...
  mutate(participant_ID = row_number(),
         Condition = recode_values(Condition, 
                                   1 ~ "Ordinary", 
                                   2 ~ "Extraordinary"),
         Gender = recode_values(Gender,
                                1 ~ "Male",
                                2 ~ "Female")) 
```
...
As a final point here, the `fill` values on the legend are not the most professional looking. You can edit the labels here through the `scale_fill_viridis_d()` layer, similar to editing the axis labels. 
...
On it's own, this does not look great. We can edit the settings to reduce the width of the boxplots, add a colour scheme, and add transparency to the layers to make it easier to see the boxplot. 
...
zhang_data_long %>% 
  ...
  geom_boxplot(width = 0.2, 
               alpha = 0.7,
               median.linewidth = 0) + # remove the median line
...
By this final plot, we have around 20 lines of <pkg>ggplot2</pkg> code. You might be worrying at this point how you will write everything from scratch when you want to create a plot. 

Like previous advice, none of this is a memory test. You will rarely write all of this and check it runs. Complex plots like this are normally an iterative process where you start with the basic components of say a violin-boxplot. Something does not look right, so you change the axis labels. Then you change the transparency of the layers. Before you know it, you have added ten edits and produced a plot you are satisfied with. Furthermore, once you have plots you are happy with, you will also find you recycle past code to adapt to new data.
:::
...
**Question 6**. Copy the following code chunk into your Quarto file and press Render. This code... works, but it does not look quite right? Why are the tick marks not displaying properly? 
...
**Question 7**. Copy the following code chunk into your Quarto file and press Render. You should receive an error like `Error in "fortify()":! "data" must be a <data.frame>, or an object coercible by "fortify()"` which is a little cryptic. 
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. We want to change the order of the categories to present males then female. This code...works, but is it doing what we think it is doing? 
...
In the next chapter, we start on inferential statistics introducing you to the concept of correlation and regression by focusing on one continuous predictor variable and one continuous outcome variable. 
