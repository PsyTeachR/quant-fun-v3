...
3. We will work within one folder, but create a new Quarto for each chapter. Create a new Quarto document and give it a sensible title describing the chapter, such as `04 Data Wrangling 1`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_04_06_datawrangling` folder. 
...
You might not just want to turn a variable into a factor, you might want to completely recode what it's values represent. For this, there is a function called `r glossary("recode_values()", def = "You can switch values from old to new. Statements are evaluated sequentially, meaning the old value is replaced with the first new value it matches.")` which you can use within `mutate()`. For example, if we wanted to make `sex` easier to interpret, we could overwrite it's values from 1 and 2: 

```{r}
sex_recode <- mutate(summarydata,
                     sex = recode_values(sex, # overwrite existing
                                         "1" ~ "Female", # old to new
                                         "2" ~ "Male")) # old to new
```
...
- We modify an existing variable `sex` by applying `recode_values()` to `sex`. 

- Within `recode_values()`, the value on the left is the existing value in the data you want to recode. The value on the right is the new value you want to overwrite it to. So, we want to change all the 1s in the data to Female. We then add a new line for every old value we want to change. 
...
Using what you just learnt about using `mutate` and `recode_values()`, recode the variable `income` and complete the code below. These are what the numeric values mean as labels: 
...
```{r eval=FALSE}
income_recode <- mutate(summarydata,
                        income = recode_values(income, # overwrite existing
...
Using a similar kind of logic to `recode_values()` we introduced you to earlier, there is an extremely flexible function called `r glossary("case_when()", def = "An if else statement to check old values against a set of criteria. Statements are evaluated sequentially, meaning each observation is checked against the criteria, and it receives the first match it passes.")` to help create new variables. Before we explain how it works, we will jump straight into an example to give you something concrete to work with.

::: {.callout-note}
#### recode_values() or case_when()?

There are actually four variations of this function designed to produce the specific results you are trying to achieve: 

1. `case_when()` creates a new vector (series of values in a variable) based on logical conditions. 

2. `replace_when()` updates an existing vector based on logical conditions. The difference here is whether you are creating completely new values or replacing old ones. 

3. `recode_values()` creates a new vector by mapping all old values to new values.

4. `replace_values()` updates an existing vector by mapping some old values to new values. Like the first two entries, the difference here is whether you are creating completely new values or replacing old ones.

It takes time to learn what function you need for a given task and do not forget: this is not a memory test. You will always be able to look up resources and past code. The important skill is knowing what you want to do with the data you are working with, you can always find the code that will help you do that. 
:::
...
**Question 4**. When you use mutate(), which additional function could you use to recode an existing variable? `r longmcq(c("arrange()", "case_when()", answer = "recode_values()", "filter()"))`

**Question 5**. When you use mutate(), which additional function could you use to create a new variable depending on specific criteria you set? `r longmcq(c("arrange()", answer = "case_when()", "recode_values()", "filter()"))`
...
Create and save a new Quarto file for these activities. Delete the example code, so your file is blank from line 6. Create a new code chunk to load `tidyverse` and the two data files: 
...
**Question 6**. Copy the following code chunk into your Quarto file and press Render. This...works, but we want 54 columns rather than 55 columns. Some of the columns do not look quite right?  
...
**Question 7**. Copy the following code chunk into your Quarto file and press Render. You should receive an error like `Error in select(): ! Can't subset columns that don't exist. x Column "interventnion" doesnt exist.` 
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. You should receive an error like:

> `Error in mutate():`
> `In argument: sex = recode_values(sex, "1" ~ "Female", "2" ~ "Male").`
> `Caused by error in recode_values():`
> `! Can't convert ..1 (left) <character> to <double>`
