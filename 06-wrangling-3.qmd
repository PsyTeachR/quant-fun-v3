...
After this chapter, you will be ready for the first data analysis journey chapter we designed for this milestone - [Data Analysis Journeys: Bartlett et al. (2022)](https://psyteachr.github.io/analysis_journeys/Bartlett-wrangle.html){target="_blank"}. This is where you can test the skills you have developed so far on a more independent task as a bridge between the core chapters and your assessments. If you want *even* more practice, you can explore the other chapters in the wrangling section of our supporting book.
...
2. Create a new Quarto document and give it a sensible title describing the chapter, such as `06 Data Wrangling 3`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_04_06_datawrangling` folder. 
...
full_data_mutate <- mutate(full_data_select,
                           GenderE = recode_values(GenderE,
                                                   1 ~ "Female",
                                                   2 ~ "Male",
                                                   3 ~ "Non-binary"),
                           RaceEthE = recode_values(RaceEthE,
                                                    1 ~ "Black/African American",
                                                    2 ~ "Hispanic/Other Latinx",
                                                    3 ~ "White",
                                                    4 ~ "Multiracial",
...
# recode items 2, 3, and 10
scales_reverse <- mutate(scales,
                         SPSS2_R = recode_values(SPSS2E,
                                                 1 ~ 5,
                                                 2 ~ 4,
                                                 3 ~ 3,
                                                 4 ~ 2,
                                                 5 ~ 1),
                         SPSS3_R = recode_values(SPSS3E,
                                                 1 ~ 5,
                                                 2 ~ 4,
                                                 3 ~ 3,
                                                 4 ~ 2,
                                                 5 ~ 1),
                         SPSS10_R = recode_values(SPSS10E,
                                                  1 ~ 5,
                                                  2 ~ 4,
                                                  3 ~ 3,
                                                  4 ~ 2,
...
One concept we will spend more time on in [Chapter 11 - Screening Data]() - is removing participants who do not provide an answer. We explore the decision making in the course materials in more detail, but there is a handy function in <pkg>tidyr</pkg> called `drop_na()`. You could do this using filter, but the standalone function streamlines things. If you run the function on your whole data set, it will remove observations with one or more NAs in all their variables: 
...
full_data_pipe <-  demog %>%
  ...
  mutate(GenderE = recode_values(GenderE,
                                 1 ~ "Female",
                                 2 ~ "Male",
                                 3 ~ "Non-binary"),
         RaceEthE = recode_values(RaceEthE,
...
We focus on <pkg>tidyverse</pkg> packages/functions in this course, so we focus on the pipe (`%>%`). This was introduced through the <pkg>magrittr</pkg> package and was popular enough that base R introduced a "native" pipe: `|>`. You might see people use this instead of the pipe but we stick with the <pkg>tidyverse</pkg> version in this book. 
...
::: {.callout-note}
#### Is there a shorter way to calculate scale scores? 

In previous chapters, we have explained we opt for longer but more readable code as you develop your understanding of R and working with data. There are often more efficient ways to achieve a goal but they involve several functions we have not covered yet or they can look a little nasty to make sense of. Now we have covered pipes, we could calculate scale scores in a few lines: 

```{r}
VSSL_sum <- full_data_mutate %>% 
  rowwise() %>% 
  mutate(VSSL_sum = sum(c_across(SPSS1E:SPSS9E), 
                        na.rm = TRUE)) %>% 
  ungroup()
```

There are two functions here we have not covered in the materials: 

- `rowwise()` is kind of like `group_by()` where it applies functions across rows rather than columns.

- `c_across()` is a helper function to apply a function to multiple columns.

So, you can read this code as take `full_data_mutate` **and then** apply functions across rows **and then** create a new variable for the sum VSSL score across items SPSS1E to SPSS9E (ignoring NAs) **and then** ungroup to stop applying functions by row. 
...
Create and save a new Quarto file for these activities. Delete the example code, so your file is blank from line 6. Create a new code chunk to load <pkg>tidyverse</pkg> and the data files: 
...
Below, we have several variations of a code chunk error or misspecification. Copy and paste them into your Quarto file below the code chunk to load <pkg>tidyverse</pkg> and the data files. Once you have copied the activities, click Render and look at the error message you receive. See if you can fix the error and get it working before checking the answer.

**Question 6**. Copy the following code chunk into your Quarto file and press Render. In this code chunk, we want to calculate the mean and SD age of all participants using `demog`. There are two errors/omissions here to try and fix: 

1. One causes the document not to render. You should receive an error like `Caused by error in "SD()": ! could not find function "SD"`.  
...
**Question 7**. Copy the following code chunk into your Quarto file and run the code. We want to calculate the sum of the five quantitative hindrances items per participant. This code... works, but does it look like it fits in the possible 5-25 range? 
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. We want to filter `demog` to focus on female participant and calculate the mean age of the female participants. You should receive an error containing `Caused by error:! "..1$StudentIDE" must be a logical vector, not <spec_tbl_df/tbl_df/tbl/data.frame> object` which is not the most helpful error for diagnosing the problem. 
...
At this point, we encourage you to complete the first data analysis journey chapter we designed for this milestone - [Data Analysis Journeys: Bartlett et al. (2022)](https://psyteachr.github.io/analysis_journeys/Bartlett-wrangle.html){target="_blank"}. This is a bridge between the structured learning in these chapters and your assessments. We present you with a new data set, show you what the end product should look like, and see if you can apply your data wrangling skills to get there. If you get stuck, we have a range of hints and steps you can unhide, then the solution to check your attempts against.
