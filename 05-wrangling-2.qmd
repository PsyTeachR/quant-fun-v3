...
In the last chapter, we introduced you to several one-table `r glossary("function", display = "functions")` from <pkg>dplyr</pkg> which we use to wrangle data. Over the course of this book, we will reinforce these functions and skills across different data sets to expose you to what psychology is about, and to reiterate that the same skills apply across different data sets. Always remember: **while the data changes, the skills stay the same!**

In this chapter, we are going to continue developing our understanding of data, and build on the knowledge and skills you have developed so far. We start with a recap of data wrangling functions from Chapter 4 and ask you to apply them to a new data set. Try and complete the activities independently to judge how well you can transfer your skills to a new scenario but remember this is not a test. If you need to look up a code example, you can always refer back to Chapter 4. We then introduce you to new data wrangling functions to filter and summarise. 

**Chapter Intended Learning Outcomes (ILOs)**

By the end of this chapter, you will be able to: 

- Apply your data wrangling skills to a new data set. 
...
2. Create a new Quarto document and give it a sensible title describing the chapter, such as `05 Data Wrangling 2`. Delete everything below line 6 so you have a blank file to work with and save the file in your `Chapter_04_06_datawrangling` folder. 
...
We often store data in two-dimensional tables, either called `r glossary("data frame", display = "data frames")`, tables, or `r glossary("tibble", display = "tibbles")`. There are other ways of storing data that you will discover in time but in this book, we will be using data frame or tibbles (a special type of data frame in the <pkg>tidyverse</pkg>). A data frame is really just a table of data with columns and rows of information. Within the cells of the data frame - a cell being where a row and a column meet - you get different types of data, including `r glossary("double")`, `r glossary("integer")`, `r glossary("character")` and `r glossary("factor")`. Within each column, the data must be one specific type. You cannot have a variable containing more than one data type. 
...
Reorder observations in the data using the following two variables: `HitOrMiss` (putting hits (1) first) and `JudgedSpeed` (putting fast judgements (1) first). Store this in an object named `arrange_dat`. 
...
pong_data <- mutate(pong_data, 
                    JudgedSpeedLabel = recode_values(JudgedSpeed, 
                                                     0 ~ "Slow",
                                                     1 ~ "Fast"))
```
...
There are actually two versions of this function: `filter()` and `filter_out()`. We will mainly use `filter()` but they are designed to avoid negating criteria. In `filter()`, any criterion evaluated to TRUE will be retained and any FALSE will be excluded. In `filter_out()`, any criterion evaluated to TRUE will be excluded and any FALSE will be retained. 

For example, if you wanted to remove trial number 1 using `filter_out()` instead, you would use:

```{r}
pong_data_filter <- filter_out(pong_data,
                               TrialNumber == 1)
```
...
For two values, this is pretty straightforward, but it could get messy when you have four or five values to evaluate against. There is a super handy shortcut from the Boolean expressions table for "in" which we can apply if we wanted to keep ball speeds of 2, 4, 5, and 7:
...
1. Rounded to 2 decimal places, the mean proportion of hits to the **blue** background was `r fitb(0.69)`, or as a percentage rounded to 0 decimal places: `r fitb(69)`%.

2. Rounded to 3 decimal places, the mean proportion of hits to the **red** background was `r fitb(0.686)` or as a percentage rounded to 1 decimal place: `r fitb(68.6)`%.
...
::: {.callout-tip}
#### Quarto tip of the chapter: Create pretty tables

After we introduced you to Quarto to create reproducible documents in Chapter 2, we are going to add a tip in every chapter to demonstrate extra functionality.

Quarto is great for embedding plots and statistics in reproducible documents, but tables can be a little tricky. If you only call objects like `hits_by_background`, the output does not look super professional and it is not consistent with APA formatting guidelines.

There are a few options available to you. One of the packages that helps create Quarto - <pkg>knitr</pkg> -  can create tables from objects you create. There is a function called `kable()` which can create tables with no further arguments, but you will need to edit the object to make sure it has headers and labels consistent with APA. The following code creates a simple table if you have <pkg>knitr</pkg> installed: 

```{r eval=FALSE}
knitr::kable(hits_by_judgedspeed)
```

You will need to Render your document to see what it looks like, but it should look similar to @fig-img-kable-demo. The row labels are fine, but you would need to tidy up the headers and round `prop_hits` to three decimals (see the function `round()`). 

```{r img-kable-demo, echo=FALSE}
#| label: fig-img-kable-demo
#| fig.cap: "Example of using kable() to create tables in Quarto."
...
Alternatively, there are other packages like <pkg>gt</pkg> ([https://gt.rstudio.com/](https://gt.rstudio.com/){target="_blank"}) or <pkg>flextable</pkg> ([https://davidgohel.github.io/flextable/](https://davidgohel.github.io/flextable/){target="_blank"}) which can also create tables with plenty of formatting options.
:::
...
During these initial learning steps, we encourage longer but more explicit code to reinforce what you are doing. There are often more concise strategies but it might be easy to miss small details. 

For example, instead of creating intermediary objects to group summaries by, there is an argument `.by = ` within `summarise()` to add a grouping variable. 

```{r}
hits_by_judgedspeed <- summarise(pong_data_filter,
                                 sum_hits = sum(HitOrMiss),
                                 prop_hits = mean(HitOrMiss),
                                 .by = JudgedSpeedLabel)
```

In the next chapter, we will demonstrate another way of avoiding intermediary objects. 
:::
...
**Question 4**. Which of the <pkg>dplyr</pkg> functions would you use to retain certain observations (e.g., keep all females)? `r longmcq(sample(c("select()", answer = "filter()", "mutate()", "count()", "group_by()", "summarise()")))` 
...
Create and save a new Quarto file for these activities. Delete the example code, so your file is blank from line 6. Create a new code chunk to load `tidyverse` and the data file: 
...
Below, we have several variations of a code chunk error or misspecification. Copy and paste them into your Quarto file below the code chunk to load `tidyverse` and the data. Once you have copied the activities, click Render and look at the error message you receive. See if you can fix the error and get it working before checking the answer.

**Question 6**. Copy the following code chunk into your Quarto file and press Render. We want to filter data to only include a paddle length of 50. You should receive the error starting with `Error in "filter()" ! We detected a named input`. 
...
**Question 7**. Copy the following code chunk into your Quarto file and press Render. We want to count the number of trials per block (`BlockNumber`). This...works, but if you look at the output, have we counted the number of trials? 
...
The mistake is using `sum()` to count the number of trials per block. `sum()` would only work when you have 0s and 1s. Here, it just adds up all the numbers, totaling 29952. There are two options here. In every other scenario, you need to either `count()`:
...
**Question 8**. Copy the following code chunk into your Quarto file and press Render. Here, we want the proportion of fast judgements per paddle length by taking the mean of `JudgedSpeed`. This code... works, but do we have a proportion of fast judgements per paddle length? 
...
Alternatively, you could use the `.by` argument to avoid the intermediary group object. 
